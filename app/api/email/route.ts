...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    await resend.emails.send({
      ...
      html: `<div style="font-family:sans-serif;max-width:500px;margin:auto;padding:32px;background:#fafaf8;border-radius:16px;">
        ...
        <div style="background:white;border-radius:12px;padding:24px;margin-bottom:16px;text-align:center;">
          <p style="font-size:32px;margin:0 0 12px;">${info.emoji}</p>
          <h3 style="color:#111;margin:0 0 8px;">Hi ${customerName || "there"}!</h3>
          <p style="color:#666;margin:0 0 16px;">${info.message}</p>
          <div style="background:#fef3c7;border-radius:8px;padding:12px;">
            <p style="font-size:11px;color:#92400e;font-weight:bold;margin:0 0 4px;">Order Number</p>
