...
export default function AdminCustomersPage() {
  ...
  useEffect(() => {
    ...
    orders.forEach((o) => {
      ...
      if (!map[key]) {
        map[key] = { name: o.customer.name, email: "", phone: o.customer.phone, orderCount: 0, totalSpent: 0, lastOrder: o.date };
      }
    ...
    // Also include registered user even if no orders
    const stored = localStorage.getItem("registeredUser");
    if (stored) {
      const user = JSON.parse(stored);
      if (!map[user.name]) {
        map[user.name] = { name: user.name, email: user.email, phone: user.phone || "", orderCount: 0, totalSpent: 0, lastOrder: "—" };
      } else {
        map[user.name].email = user.email;
      }
    }
    ...
    <AdminLayout>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="bg-white rounded-2xl shadow-sm p-4 text-center">
          ...
          <p className="text-xs text-gray-400 mt-1 font-semibold">Total Customers</p>
        </div>
        <div className="bg-white rounded-2xl shadow-sm p-4 text-center">
          <p className="text-2xl font-extrabold text-amber-600">₱{customers.reduce((s, c) => s + c.totalSpent, 0).toLocaleString()}</p>
          <p className="text-xs text-gray-400 mt-1 font-semibold">Total Revenue</p>
        </div>
        <div className="bg-white rounded-2xl shadow-sm p-4 text-center">
          <p className="text-2xl font-extrabold text-gray-900">{customers.reduce((s, c) => s + c.orderCount, 0)}</p>
          <p className="text-xs text-gray-400 mt-1 font-semibold">Total Orders</p>
        </div>
      </div>
          ...
          filtered.map((c, i) => (
            <div key={i} className="grid grid-cols-5 items-center px-5 py-3 border-b border-gray-50 hover:bg-gray-50 transition-colors">
              <div className="col-span-2 flex items-center gap-3">
                <div className="w-9 h-9 rounded-full bg-amber-100 flex items-center justify-center text-sm font-bold text-amber-600 flex-shrink-0">
