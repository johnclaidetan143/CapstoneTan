...
export default function AdminDashboardPage() {
  ...
  function updateTracking(orderNumber: string, status: typeof TRACKING[number]) {
    persist(orders.map((o) => o.orderNumber === orderNumber ? { ...o, trackingStatus: status, notificationRead: false } : o));
  }

  function updatePayment(orderNumber: string, status: string) {
  ...
  function handleBulkUpdate() {
    ...
    persist(orders.map((o) => selected.has(o.orderNumber) ? { ...o, trackingStatus: bulkStatus as typeof TRACKING[number], notificationRead: false } : o));
    setSelected(new Set()); setBulkStatus("");
  }

  function toggleSelect(orderNumber: string) {
    setSelected((prev) => { const n = new Set(prev); n.has(orderNumber) ? n.delete(orderNumber) : n.add(orderNumber); return n; });
  }

  function exportCSV() {
    const rows = [
      ["Order #", "Date", "Customer", "Phone", "Address", "City", "Items", "Total", "Payment", "Ref #", "Payment Status", "Order Status"],
      ...orders.map((o) => [o.orderNumber, o.date, o.customer.name, o.customer.phone, o.customer.address, o.customer.city,
        o.items.map((i) => `${i.name} x${i.quantity}`).join(" | "), o.total,
        o.payment.method === "gcash" ? "GCash" : "COD", o.payment.referenceNumber || "-", o.payment.status, o.trackingStatus]),
    ];
  ...
  function printInvoice(order: OrderRecord) {
    ...
    const items = order.items.map((i) => `<tr><td style="padding:6px 0;border-bottom:1px solid #f0f0f0">${i.name}</td><td style="text-align:center;border-bottom:1px solid #f0f0f0">${i.quantity}</td><td style="text-align:right;border-bottom:1px solid #f0f0f0">₱${i.price}.00</td><td style="text-align:right;border-bottom:1px solid #f0f0f0">₱${i.price * i.quantity}.00</td></tr>`).join("");
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:24px;font-size:13px"><div><p style="font-weight:bold">Bill To:</p><p>${order.customer.name}</p><p>${order.customer.phone}</p><p>${order.customer.address}, ${order.customer.city}</p></div><div><p style="font-weight:bold">Payment:</p><p>${order.payment.method === "gcash" ? "GCash" : "Cash on Delivery"}</p>${order.payment.referenceNumber ? `<p>Ref: ${order.payment.referenceNumber}</p>` : ""}<p>Status: ${order.payment.status}</p></div></div>
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    ...
                    <div className="flex flex-col gap-4">
                      ...
                      {order.payment.status !== "Verified" && order.trackingStatus !== "Cancelled" && (
                        <button onClick={() => markAsPaid(order.orderNumber)} className="bg-green-500 hover:bg-green-400 text-white text-xs font-bold px-4 py-2 rounded-full transition-colors w-fit">
                      ...
                      <div>
                        ...
                        <div className="flex gap-2 flex-wrap">
                          {["Pending Verification", "Pending Payment", "Verified", "Rejected"].map((s) => (
                            <button key={s} onClick={() => updatePayment(order.orderNumber, s)}
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {TRACKING.map((s) => (
                            <button key={s} onClick={() => updateTracking(order.orderNumber, s)}
