...
export default function MessagesPage() {
  ...
  async function fetchMessages(uid: string) {
    try {
      ...
      if (data.error) {
        console.error("[Messages] fetchMessages error:", data.error, "code:", data.code);
      ...
      console.error("[Messages] fetchMessages network error:", err);
  ...
  useEffect(() => {
    ...
    if (!isLoggedIn || !raw) { router.push("/login"); return; }

    try {
      const user  = JSON.parse(raw);
      const email = String(user?.email ?? "").trim().toLowerCase();
      const name  = String(user?.name  ?? "User").trim();
      console.log("[Messages] Logged-in user:", { email, name, id: user?.id });

      if (!email) {
        console.error("[Messages] No email in registeredUser:", raw);
        setLoading(false);
        setMounted(true);
        return;
      }
      ...
      console.error("[Messages] Failed to parse registeredUser:", err);
      setLoading(false);
      setMounted(true);
    }
  ...
  async function sendMessage() {
    ...
    try {
      const res  = await fetch("/api/chat", {
        ...
        body:    JSON.stringify({ user_id: userId, user_name: userName, message: text, sender: "user" }),
      });
      const data = await res.json();

      if (!res.ok) {
        console.error("[Messages] sendMessage failed:", res.status, data);
      ...
      console.error("[Messages] sendMessage network error:", err);
      setMessages((prev) => prev.filter((m) => m.id !== optimistic.id));
      setInput(text);
  ...
  return (
    <>
      ...
      >
        <div className="w-full max-w-[480px] flex flex-col rounded-3xl overflow-hidden shadow-2xl" style={{ height: 620 }}>

          {/* Header */}
          <div className="flex items-center gap-3 px-5 py-4 flex-shrink-0"
            style={{ background: "linear-gradient(135deg, #f48fb1 0%, #f8bbd0 100%)" }}>
            <div className="relative flex-shrink-0">
              <img src="/logo.png" alt="Cheni Craft" className="w-12 h-12 rounded-full object-cover border-2 border-white shadow" />
              <span className="absolute bottom-0 right-0 w-3 h-3 bg-green-400 border-2 border-white rounded-full" />
            </div>
            <div className="flex-1 min-w-0">
              <p className="font-extrabold text-white text-base leading-tight tracking-wide">CHENI CRAFT</p>
              <p className="text-pink-100 text-xs mt-0.5">We typically reply within a few hours</p>
            </div>
          ...
          {/* Messages area */}
          <div className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-1" style={{ background: "#fff9fb" }}>
            {loading ? (
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-center gap-3 py-8">
                <div className="w-16 h-16 rounded-full flex items-center justify-center text-3xl" style={{ background: "#fce4ec" }}>💬</div>
                ...
                <p className="text-gray-400 text-xs max-w-[220px] leading-relaxed">
                  Hi {userName}! Send us a message from the{" "}
                  <a href="/contact" className="text-pink-500 underline">Contact Us</a> page. 🌸
                </p>
              </div>
            ) : (
              messages.map((msg, i) => {
                  ...
                  <div key={msg.id}>
                    {showDate && (
                      <div className="flex justify-center my-3">
                        <span className="text-[11px] text-gray-400 px-3 py-1 rounded-full" style={{ background: "#fce4ec" }}>
                      ...
                      <div className={`max-w-[75%] flex flex-col ${isUser ? "items-end" : "items-start"}`}>
                        <div
                          className={`px-4 py-2.5 text-sm leading-relaxed whitespace-pre-wrap break-words shadow-sm ${
                            isUser ? "text-white rounded-2xl rounded-br-sm" : "text-gray-800 bg-white rounded-2xl rounded-bl-sm border border-pink-100"
                          }`}
          ...
          {/* Input area */}
          <div className="px-4 py-3 flex items-center gap-2 border-t border-pink-100 flex-shrink-0" style={{ background: "#fff" }}>
            <input
              ref={inputRef}
              type="text"
              value={input}
              ...
              disabled={sending}
              className="flex-1 rounded-full px-4 py-2.5 text-sm text-gray-800 focus:outline-none focus:ring-2 border border-pink-200 placeholder-gray-400 disabled:opacity-60"
            ...
            >
              {sending
                ...
                : <svg className="w-4 h-4 rotate-45" fill="currentColor" viewBox="0 0 24 24"><path d="M2.01 21L23 12 2.01 3 2 10l15 2-15 2z" /></svg>
              }
            </button>
          </div>
