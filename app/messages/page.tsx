...
export default function MessagesPage() {
  ...
  async function fetchMessages(email: string) {
    try {
      ...
      if (data.error) {
        console.error("[Messages] fetchMessages API error:", data.error);
            ...
            {loading ? (
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-center gap-3 py-8">
                ...
                <p className="text-gray-400 text-xs max-w-[220px] leading-relaxed">
                  ...
                  <a href="/contact" className="text-pink-500 underline">Contact Us</a> page and we&apos;ll reply soon. 🌸
                </p>
              </div>
            ) : (
              messages.map((msg, i) => {
                ...
                return (
                  <div key={msg.id}>
                    ...
                    <div className={`flex items-end gap-2 mb-0.5 ${isUser ? "flex-row-reverse" : "flex-row"}`}>
                      {!isUser && (
                        <div
                          className={`w-7 h-7 rounded-full flex-shrink-0 overflow-hidden border border-pink-200 ${showAvatar ? "visible" : "invisible"}`}
                        >
