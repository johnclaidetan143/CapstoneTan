...
export default function ContactPage() {
  ...
  const [userId, setUserId]         = useState("");   // email used as user_id
  ...
  async function fetchMessages(uid: string) {
    try {
      ...
      if (data.error) {
        console.error("[Contact] fetchMessages error:", data.error, "code:", data.code);
  ...
  useEffect(() => {
    ...
    if (isLoggedIn && raw) {
      try {
        ...
        if (!email) {
          console.error("[Contact] No email in registeredUser:", raw);
          setMounted(true);
          return;
        }
          ...
          <div className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-1" style={{ background: "#fff9fb" }}>
            {!loggedIn ? (
              <div className="flex-1 flex flex-col items-center justify-center text-center gap-3 py-8">
                ...
                <p className="text-gray-400 text-xs max-w-[220px] leading-relaxed">Please log in to send us a message.</p>
                <a href="/login" className="mt-1 px-6 py-2 rounded-full text-white text-sm font-semibold shadow transition-opacity hover:opacity-90"
                  style={{ background: "linear-gradient(135deg, #e91e8c, #f48fb1)" }}>Sign In</a>
              </div>
            ) : messages.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center text-center gap-3 py-8">
                <div className="w-16 h-16 rounded-full flex items-center justify-center text-3xl" style={{ background: "#fce4ec" }}>💬</div>
